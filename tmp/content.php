  ...
  if ( have_posts() ) {
    while ( have_posts() ) {
      ...
      <header class="article-header entry-header">
        ...
        <?php //タイトル下の広告表示
        if (is_ad_pos_below_title_visible() && is_all_adsenses_visible() && !is_front_page() ){
        ...
        <?php //SNSシェアボタン
        if (is_sns_top_share_buttons_visible() && !is_front_page() )
...
'.sprintf(__( 'この記事は<span class="bold">約%s分</span>で読めます。', THEME_NAME ), get_time_to_content_read(get_the_content())); ?></div>
        <?php endif; ?>
         ...
         <?php //本文上の広告表示
        if (is_ad_pos_content_top_visible() && is_all_adsenses_visible() && !is_front_page() ){
      ...
      <footer class="article-footer entry-footer">
        ...
        if ( is_page() && is_active_sidebar( 'above-page-sns-buttons' ) && !is_front_page() ): ?>
          <?php dynamic_sidebar( 'above-page-sns-buttons' ); ?>
        <?php endif; ?>

        <?php //SNSシェアボタン
        if (is_sns_bottom_share_buttons_visible() && !is_front_page() )
          get_template_part_with_option('tmp/sns-share-buttons', SS_BOTTOM); ?>

        <?php //SNSフォローボタン
        if (is_sns_follow_buttons_visible() && !is_front_page() )
        get_template_part_with_option('tmp/sns-follow-buttons', SF_BOTTOM); ?>

        <?php //SNSシェアボタン上の広告表示
        if (is_ad_pos_below_sns_buttons_visible() && is_all_adsenses_visible() && !is_front_page() ){
        ...
        if ( is_page() && is_active_sidebar( 'below-page-sns-buttons' )  && !is_front_page() ): ?>
